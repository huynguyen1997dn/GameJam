    ...
    public sealed class ParticleImage : MaskableGraphic
    {
        ...
        private void UpdateSheets()
        {
            ...
            if (textureSheetEnabled)
            {
                ...
                var baseSize = new Vector2(rect.z-rect.x, rect.w - rect.y);
                baseSize.x /= textureTile.x;
                baseSize.y /= textureTile.y;

                var originPos = new Vector2(rect.x, rect.y);
                ...
                for (int i = textureTile.y-1; i > -1; i--)
                {
                    for (int j = 0; j < textureTile.x; j++)
                    {
                        ...
                        Vector2 pos = originPos + offset * baseSize;
                        Vector2 size = pos + baseSize;
                        // Vector2 size = new Vector2(1f / textureTile.x * (j + 1), 1f / textureTile.y * (i + 1));
                        // Vector2 pos = new Vector2(1f / textureTile.x * j, 1f / textureTile.y * i);
                        sheetsArray[(textureTile.y - 1 - i) * textureTile.x + j] = new SpriteSheet(size, pos);
                    }
                }
            }
            else
        ...
        private void Simulate(float deltaTime, bool prewarming = false)
        {
            ...
            if (isEmitting && !canvasRenderer.cull)
            {
                ...
                //Emit over lifetime
                if (_rateOverLifetime > Mathf.Epsilon && _duration > 0)
                {
                    if ((_time < (_duration + _startDelay) || _loop) && _time >= _startDelay)
                    {
